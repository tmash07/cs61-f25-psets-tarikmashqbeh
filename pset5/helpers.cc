...
#define TYPEMASK_CONDITIONAL    ((1 << TYPE_SEQUENCE) | (1 << TYPE_EOL) | (1 << TYPE_BACKGROUND))
#define TYPEMASK_PIPELINE       (TYPEMASK_CONDITIONAL | (1 << TYPE_AND) | (1 << TYPE_OR))
#define TYPEMASK_COMMAND        (TYPEMASK_PIPELINE | (1 << TYPE_PIPE))
...
int shell_parser::next_op() const {
    shell_tokenizer it(_stop, _end);
    return it.type();
}

const char* shell_parser::next_op_name() const {
    shell_tokenizer it(_stop, _end);
    return it.type_name();
}

conditional_parser command_line_parser::conditional_begin() const {
    return first_delimited(TYPEMASK_CONDITIONAL);
}

pipeline_parser command_line_parser::pipeline_begin() const {
    return first_delimited(TYPEMASK_PIPELINE);
}

pipeline_parser conditional_parser::pipeline_begin() const {
    return first_delimited(TYPEMASK_PIPELINE);
}

command_parser command_line_parser::command_begin() const {
    return first_delimited(TYPEMASK_COMMAND);
}

command_parser conditional_parser::command_begin() const {
    return first_delimited(TYPEMASK_COMMAND);
}

command_parser pipeline_parser::command_begin() const {
    return first_delimited(TYPEMASK_COMMAND);
}

void conditional_parser::operator++() {
    next_delimited(TYPEMASK_CONDITIONAL);
}

void pipeline_parser::operator++() {
    next_delimited(TYPEMASK_PIPELINE);
}

void command_parser::operator++() {
    next_delimited(TYPEMASK_COMMAND);
}
...
shell_parser shell_parser::first_delimited(unsigned long fl) const {
    ...
    while (it._s > _s && isspace((unsigned char) it._s[-1])) {
        --it._s;
    }
...
int claim_foreground(pid_t pgid) {
    ...
    return tcsetpgrp(ttyfd, pgid ? pgid : shell_pgid);
}
