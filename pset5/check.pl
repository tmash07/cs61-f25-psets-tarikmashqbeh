...
sub run_sh61 ($;%) {
    ...
    my ($sigint_state) = defined($opt{"int_delay"}) ? 1 : 0;
    ...
    my ($sigint_at) = $sigint_state > 0 ? $before + $opt{"int_delay"} : undef;
    my ($died) = 0;
...
sub run (@) {
    ...
    return if $opts{CMD_SKIP} && !@ALLOW_TESTS && !$NO_SKIP;
