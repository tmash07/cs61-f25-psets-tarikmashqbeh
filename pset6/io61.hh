...
struct io61_args {
    ...
    const char* program_name = nullptr; // name of program
    const char* opts = nullptr;         // options string
    ...
    static std::optional<size_t> parse_size(const char* str, size_t min = 0);
    void usage();
...
inline ssize_t io61_read_bytewise(io61_file* f, char* buf, size_t sz) {
    return io61_read_bytewise(f, reinterpret_cast<unsigned char*>(buf), sz);
}

inline ssize_t io61_write_bytewise(io61_file* f, const char* buf, size_t sz) {
    return io61_write_bytewise(f, reinterpret_cast<const unsigned char*>(buf), sz);
}
