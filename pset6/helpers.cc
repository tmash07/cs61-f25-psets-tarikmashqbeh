...
std::optional<size_t> io61_args::parse_size(const char* s, size_t min) {
    ...
    } else if (ptr == s || (v == ULONG_MAX && errno == ERANGE) || v < min) {
        return std::nullopt;
    } else if (ptr == ends) {
        return v;
    }
    ...
    if (round(fv) != fv || fv > SIZE_MAX || fv < min) {
        return std::nullopt;
    }
    return fv;
}
...
io61_args& io61_args::parse(int argc, char** argv) {
    ...
    while ((arg = getopt(argc, argv, this->opts)) != -1) {
        ...
        case 's':
            ...
            break;
        case 'b':
            if (auto sz = parse_size(optarg, 1)) {
                bs = *sz;
            } else {
                goto usage;
            }
