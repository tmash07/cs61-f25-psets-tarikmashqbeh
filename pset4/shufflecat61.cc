...
// Usage: ./shufflecat61 [-b BLOCKSIZE] [-r RANDOMSEED] [-s SIZE]
//                       [-o OUTFILE] [FILE]
//    Copies the input FILE to OUTFILE in blocks. The blocks are
//    read in random order, and written in the same order, but
//    the resulting output file should be the same as the input.
...
int main(int argc, char* argv[]) {
    ...
    if (io61_seek(inf, 0) < 0) {
        fprintf(stderr, "shufflecat61: input file is not seekable\n");
        exit(1);
    }
    io61_file* outf = io61_open_check(args.output_file,
                                      O_WRONLY | O_CREAT | O_TRUNC);
    if (io61_seek(outf, 0) < 0) {
        fprintf(stderr, "shufflecat61: output file is not seekable\n");
        exit(1);
    }
    ...
    if (ssize_t(args.file_size) < 0) {
        fprintf(stderr, "shufflecat61: can't get size of input file\n");
        exit(1);
    }
    ...
    if (nblocks > (30 << 20)) {
        fprintf(stderr, "shufflecat61: file too large\n");
        exit(1);
    } else if (nblocks * args.block_size != args.file_size) {
        fprintf(stderr, "shufflecat61: input file size not a multiple of block size\n");
        exit(1);
    }
