...
sub run_sh61 ($;%) {
    ...
    if (exists($answer->{"status"})
        ...
        $signame = defined($signame) ? $signame : "signal $signo";
...
sub enqueue ($$$%) {
    ...
    my $check_random = exists($opt{"check_random"}) ? $opt{"check_random"} : 0;
    my $compare;
    ...
    my $stdio_qitem = {
        ...
        "compare" => $compare, "check_random" => 0,
        "perf" => $perf
    };
...
sub run_qitem ($) {
    ...
    # find cached test if there is one
    my $result;
    for (my $i = 0; $i < @cachedtests; ++$i) {
        my $t = $cachedtests[$i];
        if ($t->{"id"} eq $qitem->{"id"}
            && $t->{"type"} eq $qitem->{"type"}
            && $t->{"qcommand"} eq $qitem->{"command"}) {
            splice @cachedtests, $i, 1;
            $result = $t;
            last;
        }
    }
    ...
    $qitem->{"count"} += 1;
    $qitem->{"errors"} += 1 if exists($result->{"killed"});
    $qitem->{"elapsed"} += $result->{"time"};

    if (!$max_size
        && exists($result->{"outputsize"})
        && $result->{"outputsize"} > $command_max_size{$maincommand}) {
        $command_max_size{$maincommand} = $result->{"outputsize"};
    }
...
sub check_trial_output ($$$) {
    ...
    if (!exists($tt->{"different_content"})
        && !$param{"NOYOURCODE"}
        && $qitem->{"check_random"}) {
        my (@random_check) = @{$qitem->{"outfiles"}};
        foreach my $fname (@random_check) {
            my $rootfname = $fname;
            $rootfname =~ s/\b(inputs|outputs|stdoutputs)\//${ROOT}$1\//;
            my $r = scalar(`./randcheck61 $rootfname`);
            if ($? && $r) {
                $tt->{"different_content"} = "$fname differs from expectation, should be random";
                last;
            }
        }
    }
...
sub print_stdio ($) {
    ...
    if (exists($t->{"utime"})) {
        printf("%.5fs (%.5fs user, %.5fs system, %.0fMiB memory, %d %strial%s)\n",
               $t->{"time"}, $t->{"utime"}, $t->{"stime"}, $maxrss / 1024.0,
               $t->{"medianof"},
               $t->{"ncached"} ? "cached " : "",
               $t->{"medianof"} == 1 ? "" : "s");
    } else {
        printf("${Red}KILLED${Redctx} after %.5fs (%d %strial%s)${Off}\n",
               $t->{"time"},
               $t->{"medianof"},
               $t->{"ncached"} ? "cached " : "",
               $t->{"medianof"} == 1 ? "" : "s");
...
sub check_trial_errors ($$) {
    ...
    if (exists($tt->{"different_size"})) {
        ...
        print "    ${Red}ERROR: ", join("+", @{$qitem->{"outfiles"}}), " has size ", $size, ", expected ", $tt->{"different_size"}, "${Off}\n";
        $error = 1;
    }
    if (exists($tt->{"different_content"})) {
    ...
    if ($VERBOSE || $param{"MAKETRIALLOG"} || $param{"CACHE"}) {
        ...
        if (($param{"MAKETRIALLOG"} || $param{"CACHE"}) && @testjsons) {
            ...
            if ($param{"MAKETRIALLOG"}) {
                $outfile = $param{"MAKETRIALLOG"} eq "1" ? "triallog.txt" : $param{"MAKETRIALLOG"};
            } else {
                $outfile = ".deps/stdioresults.jsonseq";
                $mode = "+<" if -f $outfile;
            }
            open(OTRIALLOG, $mode, $outfile) or die "Cannot open $outfile";
            truncate(OTRIALLOG, 0);
            print OTRIALLOG "\x1e", join("\x1e", @testjsons);
            close(OTRIALLOG);
        }
    }
}
...
%param = (
    ...
    "CACHE" => nonemptyenv("CACHE") ? boolenv("CACHE") : 1,
...
$FILECHECKSUM = $VERBOSE || $param{"MAKETRIALLOG"} || defined($param{"TRIALLOG"}) || $param{"CACHE"};

# maybe read a trial log
if (defined($param{"TRIALLOG"})) {
    read_triallog($param{"TRIALLOG"});
} elsif (defined($param{"CACHE"})) {
    if (-r ".deps/stdioresults.jsonseq"
        && -C ".deps/stdioresults.jsonseq" < 0.25) {
        print STDERR "READING STDIO RESULTS\n";
        read_triallog(".deps/stdioresults.jsonseq");
    }
}
...
my ($binsm) = register_file("inputs/binary40k.bin", 4 << 9);
my ($binmd) = register_file("inputs/binary5m.bin", 4 << 9);
my ($textmd) = register_file("inputs/text12m.txt", 5 << 9);
...
enqueue("C12",
    "./varblockcat61 -b 256 -o outputs/out.txt $texttiny",
    "variable block I/O, sequential correctness",
    "perf" => 0, "expect" => $texttiny);

enqueue("C13",
    "./varblockcat61 -b 256 -X -o outputs/out.txt $texttiny",
    "variable block I/O, sequential correctness",
    "perf" => 0, "expect" => $texttiny);
...
enqueue("C19",
    "./reverse61 -s 500000 -o outputs/out.txt /dev/zero",
    "magic zero file, byte I/O, sequential",
    "perf" => 0, "insize" => 500000);

enqueue("C20",
    "./reverse61 -s 400000 -o outputs/out.txt /dev/urandom",
    "magic random file, byte I/O, sequential",
    "perf" => 0, "compare" => -1, "insize" => 400000, "check_random" => 1);


# NONSEQUENTIAL CORRECTNESS
enqueue("CN1",
...
enqueue("CN7",
    "./reverse61 -s 500000 -o outputs/out.txt /dev/urandom",
    "magic random file, byte I/O, reverse order",
    "perf" => 0, "compare" => -1, "insize" => 500000, "check_random" => 1);


# REGULAR FILES, SEQUENTIAL I/O
enqueue("MP1",
    "./blockcat61 -b 1024 -o outputs/out.txt $textmd",
    "regular medium file, 1KiB block I/O, sequential");

enqueue("MP2",
    "./blockcat61 -b 8192 -o outputs/out.txt $textmd",
    "regular medium file, 8KiB block I/O, sequential");

enqueue("MP3",
    "./blockcat61 -R -b 8192 -o outputs/out.txt $textmd",
    "regular medium file, byte reads/8KiB block writes, sequential");
...
enqueue("MP5",
    "./blockcat61 -o outputs/out.bin $binmd",
    "regular medium binary file, 4KiB block I/O, sequential");
...
enqueue("MP7",
    ...
    "magic file, byte I/O, sequential",
    "insize" => 5242880);

enqueue("MP8",
    "cat $textmd | ./blockcat61 -b 1024 | cat > outputs/out.txt",
    "piped medium file, 1KiB block I/O, sequential");

enqueue("MP9",
    "./blockcat61 -b 1024 $textmd | cat > outputs/out.txt",
    "mixed-piped medium file, 1KiB block I/O, sequential");
...
enqueue("MPN2",
    "./reverse61 -A 10m -o outputs/out.txt $textmd",
    "regular medium file, byte I/O, reverse order, limited memory");

enqueue("MPN3",
    "./reverse61 -s 5m -o outputs/out.txt /dev/zero",
    "magic file, byte I/O, reverse order",
    "insize" => 5242880);

enqueue("MPN4",
    "./stridecat61 -t 3m -o outputs/out.txt $textmd",
    "regular medium file, byte I/O, 3MiB stride order");

enqueue("MPN5",
    "./stridecat61 -A 10m -t 3m -o outputs/out.txt $textmd",
    "regular medium file, byte I/O, 3MiB stride order, limited memory");
...
enqueue("MPN7",
    "./endorder61 -RW -B 4096 -o outputs/out.txt $textmd",
    "regular medium file, byte I/O, seek table order");

enqueue("MPN8",
    "./endorder61 -A 10m -RW -B 4096 -o outputs/out.txt $textmd",
    "regular medium file, byte I/O, seek table order, limited memory");
...
enqueue("LP2",
    ...
    "regular large file, 4KiB block I/O, sequential");

enqueue("LP3",
    "./varblockcat61 -o outputs/out.txt $textlg",
    "regular large file, 1B-4KiB variable block I/O, sequential");

enqueue("LP4",
    "./varblockcat61 -r 6582 -o outputs/out.txt $textlg",
    "regular large file, 1B-4KiB variable block I/O, sequential");
...
enqueue("LP6",
    ...
    "piped large file, 4KiB block I/O, sequential");

enqueue("LP7",
    "cat $textlg | ./varblockcat61 | cat > outputs/out.txt",
    "piped large file, 1B-4KiB variable block I/O, sequential");

enqueue("LP8",
    "cat $textlg | ./blockcat61 -o outputs/out.txt",
    "mixed-piped large file, 4KiB block I/O, sequential");

enqueue("LP9",
    "./varblockcat61 $textlg > outputs/out.txt",
    "redirected large file, 1B-4KiB variable block I/O, sequential");


# LARGE NONSEQUENTIAL
enqueue("LPN1",
    "./reverse61 -s 8m -o outputs/out.txt $textlg",
    "regular large file, byte I/O, reverse order");

enqueue("LPN2",
    "./shufflecat61 -o outputs/out.txt $textlg",
    "regular large file, 4KiB block I/O, random seek order");

enqueue("LPN3",
    "./shufflecat61 -r 6582 -o outputs/out.txt $textlg",
    "regular large file, 4KiB block I/O, random seek order");

enqueue("LPN4",
    "./shufflecat61 -A 10m -r 6582 -o outputs/out.txt $textlg",
    "regular large file, 4KiB block I/O, random seek order, limited memory");

enqueue("LPN5",
    "./endorder61 -RW -B 4096 -o outputs/out.txt $textlg",
    "regular large file, byte I/O, seek table order");

enqueue("LPN6",
    "./endorder61 -A 10m -RW -B 4096 -o outputs/out.txt $textlg",
    "regular large file, 1-4KiB block I/O, seek table order, limited memory");
