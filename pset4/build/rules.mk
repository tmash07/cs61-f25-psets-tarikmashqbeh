...
ifeq ($(COMPILER),clang)
 ifeq ($(origin CC),default)
  ifeq ($(shell if clang --version 2>&1 | grep -e 'LLVM\|clang' >/dev/null; then echo 1; else echo 0; fi),1)
CC = clang
  endif
 endif
...
ISCLANG := $(shell if $(CC) --version 2>&1 | grep -e 'LLVM\|clang' >/dev/null; then echo 1; else echo 0; fi)
ifeq ($(ISCLANG),1)
BADCXXFLAGS ?= -fno-if-conversion -fno-if-conversion2
endif

ifeq ($(NEED_CXX_GCC),1)
GXX_ISCLANG := $(shell if g++ --version 2>&1 | grep -e 'LLVM\|clang' >/dev/null; then echo 1; else echo 0; fi)
 ifeq ($(GXX_ISCLANG),1)
  ifeq ($(shell if g++-14 --version 2>&1 | grep -e 'Free Software' >/dev/null; then echo 1; else echo 0; fi),1)
CXX_GCC = g++-14
  else ifeq ($(shell if g++-13 --version 2>&1 | grep -e 'Free Software' >/dev/null; then echo 1; else echo 0; fi),1)
CXX_GCC = g++-13
  else
CXX_GCC = false
  endif
